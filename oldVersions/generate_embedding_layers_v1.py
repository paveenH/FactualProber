...
def save_data(df, embeddings, output_path: Path, dataset_name: str, model_name: str, remove_period: bool):
    ...
    output_file = output_path / f"embeddings_{dataset_name}_{sanitized_model_name}{filename_suffix}.csv"
    # Ensure embeddings length matches DataFrame length
    if len(embeddings) != len(df):
        logging.error(f"Length of embeddings ({len(embeddings)}) does not match length of DataFrame ({len(df)}).")
        return
    # Add embeddings to the DataFrame as JSON strings
    df = df.reset_index(drop=True)
    df["embeddings"] = [json.dumps(embedding) for embedding in embeddings]
